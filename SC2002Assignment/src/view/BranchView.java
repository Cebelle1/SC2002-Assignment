...
public class BranchView extends ARenderView{
    ...
    public void displayOpenBranch(List<Branch> branches, boolean employee){ //employee=0 > customer, employee=1 > employee
        if(!employee){
    ...
    public void displayAllBranch(List<Branch> branches){    //pass in the full branch here
        super.printBorder("Logged in as Employee");
        int i=0;
        for(Branch branch: branches){
            if(branch.getOperation()==true){
